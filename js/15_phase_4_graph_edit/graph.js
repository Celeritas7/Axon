...
import {
  LEVEL_SHAPES, LEVEL_FONT_SIZES, LEVEL_FONT_WEIGHTS,
  GROUP_COLORS, FASTENER_COLORS, SEQUENCE_BADGES,
  NODE_WIDTH_BASE, NODE_WIDTH_MAX, LEVEL_HORIZONTAL_GAPS,
  getLevelShape, getLevelFontSize, getLevelFontWeight,
...
// ============================================================
// GET NODE STYLE — Datum tokens (fill + stroke), theme-aware
// ============================================================
// App status enum → Datum status CSS token (used by node fill + status dot)
const STATUS_TOKENS = {
  DONE: '--st-done',
  IN_PROGRESS: '--st-wip',
  BLOCKED: '--st-blocked',
...
function getNodeStyle(node) {
  ...
  if (colorMode === 'status') {
    const tok = STATUS_TOKENS[node.status] || '--ink-3';
    return {
      fill: `color-mix(in srgb, var(${tok}) 16%, var(--surface))`,
      stroke: `var(${tok})`
    };
  }

  if (colorMode === 'group') {
    const c = GROUP_COLORS[(node.group_num || 0) % GROUP_COLORS.length];
    return { fill: c, stroke: darkenColor(c, 35) };
  }

  // Default: level coloring → Datum --lvN palette (soft fill + saturated bar stroke)
  const n = Math.min(Math.max(node.level || 1, 1), 8);
  return { fill: `var(--lv${n}-fill)`, stroke: `var(--lv${n}-bar)` };
}
...
function drawShape(selection, shapeType, width, height, fill, stroke, isMultiParent, isOrphan) {
  ...
  // Datum: soft level/status fill + saturated token stroke (orphans get the violet token)
  const finalStroke = isOrphan ? 'var(--st-orphan)' : stroke;

  selection.selectAll('.node-shape, .multi-parent-indicator').remove();

  // Apply shared paint (fill/stroke via .style so CSS var()/oklch tokens resolve)
  const paint = (sel) => sel
  ...
  const rect = (rx, ry) => {
    const r = selection.insert('rect', ':first-child')
      .attr('x', -halfW).attr('y', -halfH).attr('width', width).attr('height', height)
      .attr('rx', rx);
    if (ry != null) r.attr('ry', ry);
    return paint(r);
  };
  const poly = (pts) => paint(
    selection.insert('polygon', ':first-child').attr('points', pts)
  );

  const shapes = {
    rectangle: () => rect(0),
    rounded_rectangle: () => rect(6),
    stadium: () => rect(halfH, halfH),
    parallelogram: () => {
      const sk = 10;
      poly(`${-halfW+sk},${-halfH} ${halfW+sk},${-halfH} ${halfW-sk},${halfH} ${-halfW-sk},${halfH}`);
    },
    pentagon: () => {
      const a = (2*Math.PI)/5, sa = -Math.PI/2, rx = halfW*0.85, ry = halfH*0.9;
      poly(Array.from({length:5}, (_,i) => `${rx*Math.cos(sa+i*a)},${ry*Math.sin(sa+i*a)}`).join(' '));
    },
    hexagon: () => {
      poly(`${-halfW+10},${-halfH} ${halfW-10},${-halfH} ${halfW},0 ${halfW-10},${halfH} ${-halfW+10},${halfH} ${-halfW},0`);
    },
    diamond: () => {
    ...
    octagon: () => {
      ...
      poly(`${-halfW+c},${-halfH} ${halfW-c},${-halfH} ${halfW},${-halfH+c} ${halfW},${halfH-c} ${halfW-c},${halfH} ${-halfW+c},${halfH} ${-halfW},${halfH-c} ${-halfW},${-halfH+c}`);
    }
  };
  ...
  if (isMultiParent) {
    // Multi-parent indicator: accent outline ring
    selection.insert('rect', ':first-child').attr('class', 'multi-parent-indicator')
      ...
      .attr('rx', 4)
      .style('fill', 'none').style('stroke', 'var(--accent)').attr('stroke-width', 1.5);
  }
}
...
export function renderGraph() {
  ...
  linkGroups.each(function(linkData) {
    ...
    const pathEl = group.append('path')
      .attr('class', 'link')
      .attr('d', pathD)
      .style('stroke', linkData.fastener ? fastenerColor : 'var(--link)')
    ...
    labelGroup.append('rect')
      ...
      .style('fill', hasLabel ? 'var(--surface)' : 'transparent')
      ...
      .on('mouseover', function() {
        if (state.isAdmin && hasLabel) {
          d3.select(this).style('opacity', 1).style('stroke', 'var(--accent)').attr('stroke-width', 1);
        }
      })
  ...
  nodeElements.each(function(d) {
    const group = d3.select(this);
    const { fill: nodeFill, stroke: nodeStroke } = getNodeStyle(d);
    ...
    // Draw shape
    drawShape(group, shape, nodeW, nodeH, nodeFill, nodeStroke, isMultiParent, isOrphan);
    ...
    // Status indicator dot (top-left, visible to all) — Datum status tokens
    const statusTok = STATUS_TOKENS[d.status] || '--ink-3';
...
export { calculateLevels, getNodeStyle, isNodeVisible, drawShape };
